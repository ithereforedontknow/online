...
";
    ...
    public function getNotificationCount($data)
    {
        try {
            ...
            $stmt = $this->conn->prepare("SELECT COUNT(*) FROM transaction WHERE created_at >= :currentTime AND updated_at >= :currentTime AND origin_id = :branch");
...
try {
    ...
    $branchManager = new branchManager($conn);

    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $action = $_POST['action'] ?? '';
        switch ($action) {
            case 'get-transaction-count':
                $branchManager->getTransactionCount($_POST);
                break;
            case 'get-notifications':
                $branchManager->getNotifications($_POST);
                break;
            case 'get-notification-count':
                $branchManager->getNotificationCount($_POST);
                break;
            default:
                $branchManager->sendResponse(false, 'Invalid action');
        }
    } else {
        $branchManager->sendResponse(false, 'Method not allowed');
    }
} catch (Exception $e) {
    error_log('Unhandled error: ' . $e->getMessage());
    echo json_encode([
