...
$departed = $conn->prepare("SELECT count(*) FROM transaction where transaction.status = 'departed' && transaction.origin_id = '$origin'");
$departed->execute();
$Departed = $departed->fetch(PDO::FETCH_ASSOC);

$arrived = $conn->prepare("SELECT count(*) FROM transaction where transaction.status = 'arrived' && transaction.origin_id = '$origin'");
$arrived->execute();
$Arrived = $arrived->fetch(PDO::FETCH_ASSOC);

$queue = $conn->prepare("SELECT count(*) FROM queue inner join transaction on queue.transaction_id = transaction.transaction_id where status = 'queue' OR status = 'standby' OR status = 'standby - sms sent' && transaction.origin_id = '$origin'");
$queue->execute();
$Queue = $queue->fetch(PDO::FETCH_ASSOC);

$ongoing = $conn->prepare("SELECT count(*) FROM transaction where transaction.status = 'ongoing' && transaction.origin_id = '$origin'");
$ongoing->execute();
